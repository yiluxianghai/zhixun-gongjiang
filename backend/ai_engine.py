...
async def analyze_problem(
    ...
    if _provider != "rule_engine" and _api_key:
        # 将静态知识库的专业内容合并到RAG上下文中，作为LLM的参考
        combined_context = rag_context or ""
        text = f"{raw_description} {inspection_area}"

        # 从静态知识库中提取相关整改模板和风险规则
        static_hints = []
        cat_code, cat_name = _kb.classify_category(text)
        spec_code, spec_name = _kb.classify_specialty(text)
        risk_level, risk_reason = _kb.determine_risk_level(text, cat_code)
        risk_rule = _kb.get_risk_rule(risk_level)
        static_hints.append(
            f"【规则引擎预判】类别: {cat_name}，专业: {spec_name}，"
            f"风险等级: {risk_level}，整改期限: {risk_rule['deadline_days']}天"
        )
        matched_template = _kb.match_template(text, cat_code, spec_code)
        if matched_template:
            tpl_req = matched_template.get("rectification_req", "")
            if tpl_req:
                static_hints.append(f"【整改模板参考】{tpl_req}")

        if static_hints:
            static_block = "【专家知识参考】\n" + "\n".join(static_hints)
            combined_context = f"{combined_context}\n\n{static_block}" if combined_context else static_block
        ...
        llm_result = await call_llm(
            ...
            rag_context=combined_context if combined_context else None,
        )
        if llm_result:
            # 补充规则引擎的整改期限（如果LLM未返回）
            if "rectification_deadline_days" not in llm_result:
                llm_result["rectification_deadline_days"] = risk_rule["deadline_days"]

            # 规范化字段类型：LLM可能返回list而非str，需统一为str
            for field in ("rectification_req", "review_points", "risk_reason",
                          "standardized_description", "responsible_party"):
                val = llm_result.get(field)
                if isinstance(val, list):
                    llm_result[field] = "\n".join(str(v) for v in val)

            rag_status = '有' if combined_context else '无'
            confidence = llm_result.get('confidence', 'N/A')
            print(f"[AI引擎] LLM分析完成（知识库={rag_status}），置信度: {confidence}")
            return llm_result
